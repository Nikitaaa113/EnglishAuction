...
EffectsAuction auc = new EffectsAuction();
      ...
      for (int k=0;k<=r.getArrayProduct().size()-1;k++) {
          System.out.println("____________"); 
     
       auc.game(k, r.getArrayProduct().get(k), r, c);   
       System.out.println("Лот:" + r.getArrayProduct().get(k).getLot() + " Предполагаемая цена:" + r.getArrayProduct().get(k).getEstimated_price());
        ...
        //c.calculate_emoticons(r, j, i, auc);
          //  System.out.println("Имя:" + r.getArrayPlayer().get(j).getName() + " Лот:" + r.getArrayProduct().get(i).getLot() + " Решение:" + r.getArrayProduct().get(i).getD_estimated_price() + " Цена:" + r.getArrayProduct().get(i).getStartPrice() + " Предполагаемая цена:" + r.getArrayProduct().get(i).getEstimated_price() + " Разница цен:" + r.getArrayProduct().get(i).getPrice_difference() + " Приемлимость:" + r.getArrayPlayer().get(j).getPrice_acceptability());
            ...
            // System.out.println("Уверенность в себе:" + r.getArrayPlayer().get(j).getSelf_confidence());
                //   System.out.println("Д Уверенность:" + r.getArrayPlayer().get(j).getD_self_confidence());
       //          System.out.println("Страх потери:" + r.getArrayPlayer().get(j).getD_fear_of_loss());
